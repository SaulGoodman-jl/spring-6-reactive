...
import guru.springframework.spring6reactive.srevices.BeerService;
...
public class BeerController {
    ...
    Mono<ResponseEntity<Void>> createNewBeer(@Validated @RequestBody BeerDTO beerDTO) {
        return beerService.saveBeer(beerDTO).map(saveDto -> ResponseEntity.created(
                UriComponentsBuilder.fromHttpUrl("http://localhost:8080/" + BEER_PATH + "/" + saveDto.getId())
                        .build().toUri()).build());
    }
